...
class SalesController extends Controller
{
    ...
    public function getTotalAmount(Request $request)
    {
        ...
        $mdx = "
            ...
            {[Date].[Year].&[{$timeLevel}] : [Date].[Year].&[$timeLevel]} ON ROWS
            from [SalesCube]
        ";
        try {
            $rawXml = $this->xmlaService->executeMdx($mdx);
    ...
    public function getTopSalespeople(Request $request)
    {
        $measures = $request->measures ?? ['[Measures].[Sales Amount]'];
        $timeLevel = $request->time_level ?? '[Date].[Year]';
        ...
        $mdx = "
            SELECT
                {$mdxMeasures} ON COLUMNS,
                NON  [Sales Person].[EmployeeName].Members ON ROWS
            FROM [SalesCube]
    ...
    public function byCategory(Request $request)
    {
        ...
        $mdx = "
            SELECT
                {$mdxMeasures} ON COLUMNS,
                NON EMPTY [Sales Person].[EmployeeName].Members ON ROWS
            FROM [SalesCube]
            WHERE ([Date].[Year].&[2024])
        ";
    ...
    public function byTerritory(Request $request)
    {
        $measures = $request->measures ?? ['[Measures].[Sales Amount]'];
        $timeLevel = $request->time_level ?? '[Date].[Year]';
